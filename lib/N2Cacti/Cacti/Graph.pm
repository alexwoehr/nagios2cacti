...
# The constructor
#
# @args		: the parameters { tables, hostname, service_description, graph_item_type, rrd, source, graph_item_colors }
# @return	: the object
#
sub new {
	my $class	= shift;
...
sub create_instance {
	...
	eval {
		...
		if ( not $this->database->db_fetch_hash("graph_local", { host_id => $hostid, graph_template_id => $gt->{id}}) ) {
		...
		if ( not $this->database->item_exist("graph_templates_graph", { graph_template_id => $gt->{id}, local_graph_id => $gl->{id}}) ) {
			...
			$gtg->{t_image_format_id} = 0;
			$gtg->{t_title} = 0;
			$gtg->{t_height} = 0;
			$gtg->{t_width} = 0;
			$gtg->{t_upper_limit} = 0;
			$gtg->{t_lower_limit} = 0;
			$gtg->{t_vertical_label} = 0;
			$gtg->{t_slope_mode} = 0;
			$gtg->{t_auto_scale} = 0;
			$gtg->{t_auto_scale_opts} = 0;
			$gtg->{t_auto_scale_log} = 0;
			$gtg->{t_scale_log_units} = 0;
			$gtg->{t_auto_scale_rigid} = 0;
			$gtg->{t_auto_padding} = 0;
			$gtg->{t_base_value} = 0;
			$gtg->{t_grouping} = 0;
			$gtg->{t_export} = 0;
			$gtg->{t_unit_value} = 0;
			$gtg->{t_unit_exponent_value} = 0;
...
sub create_template {
	...
	eval {
		...
		if ( not $this->database->item_exist("graph_templates",{ hash => generate_hash("graph_templates : " . $this->{graph_template_name})} ) ) {
		...
		if ( not $this->database->item_exist("graph_templates_graph", { graph_template_id => $gt->{id}, local_graph_template_graph_id => 0, local_graph_id => 0}) ) {
...
#
# @args		: the init color code
# @return	: the color_item_id
#
sub get_random_color {
	...
	do {
		...
		if($init_colors>=0 && $init_colors<scalar(@colors)){
			...
			if ( not$this->database->item_exist("colors", { id=> $id}) ) {
				$id=int(rand(100))+4;
			}
		}
		else{
	...
	while( not $this->database->item_exist("colors", { id=> $id}) );
	return $id;
}
...
sub update_input {
	...
	my ($hostid, $gl, $gt, $gtg, $gt_input, $ds_name);
	...
	eval {
		...
		foreach my $ds_name ( keys %$datasource ) {
			Main::log_msg("N2Cacti::Cacti::Graph::update_input(): ds.rrd_file=$datasource->{$ds_name}->{rrd_file}", "LOG_DEBUG");
			...
			$sequence = 1 if not defined($sequence);
...
#			my $dtr	= $this->database->db_fetch_hash("data_template_rrd", { hash => generate_hash($ds->{ds_name}.generate_hash($this->{data_template_name})) });
			my $dtr	= $this->database->db_fetch_hash("data_template_rrd", { data_source_name => $ds_name, local_data_id => 0, local_data_template_rrd_id => 0 });
#			my $gti = $this->database->db_fetch_hash("graph_templates_item", { hash => generate_hash("graph_template_id $$ds{ds_name} sequence $sequence")} );
			my $gti;
			my $gt_input_hash = generate_hash("graph_template_input : $ds_name graph_template_id : $$gt{id}");
			...
			Main::log_msg("N2Cacti::Cacti::Graph::update_input(): create input for the ds_name [$ds_name]", "LOG_DEBUG");
			$gt_input = $this->database->new_hash("graph_template_input");
			delete($gt_input->{id});

			$gt_input->{hash} = $gt_input_hash;
			$gt_input->{column_name} = "task_item_id";
			$gt_input->{name} = "Data Source [$ds_name]";
			...
			$gti->{id}				= "0";
			$gti->{hash}				= generate_hash("graph_template_id $ds_name sequence $sequence");
			...
			my $dt = $this->database->db_fetch_hash("data_template", { name => "$this->{data_template_name} - $ds_name" });
			my $dl;
			...
			$this->database->commit();
#			$this->database->rollback() if $@;
			Main::log_msg("N2Cacti:/Cacti:/Graph::update_input(): $@", "LOG_ERR") if $@;
			$this->database->begin();
			...
			if ( $this->database->item_exist( "graph_local", { host_id => $hostid, graph_template_id => $gt->{id} } ) ) {
				...
				while ( my $gti = $sth->fetchrow_hashref() ) {
					...
					if ( not $this->database->item_exist("graph_templates_item",{ local_graph_template_item_id => $gti->{id}, local_graph_id => $gl->{id}, task_item_id => $dtr_instance->{id} || "0", sequence => $gti->{sequence}})){
