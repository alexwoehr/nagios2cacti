...
#	log_msg("no template found - you need to configure cacti and create a host template to nagios host - report to INSTALL file, cacti configuration") if $this->{$debug};
...
sub add_device{
	...
	Main::log_msg("N2Cacti::Cacti::add_device(): add device '$hostname' to '$address' in cacti", "LOG_DEBUG") if $debug;;
}
